...
GITHUB_USER = "kanhaiyaraikavara39-pixel"      # <--- अपना गिटहब यूजरनेम डालें
GITHUB_REPO = "KR-LIKE"            # <--- अपनी रिपॉजिटरी का नाम डालें
GITHUB_TOKEN = "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"        # <--- अपना गिटहब पर्सनल एक्सेस टोकन डालें
...
# ग्लोबल वैरिएबल्स जो गिटहब से सिंक होंगे
bot_status = "on"
daily_stats = {}
user_limits = {}
...
# ============ 🐙 GITHUB STORAGE FUNCTIONS (SYNC ENGINE) ============

async def fetch_from_github():
    """गिटहब से database.json का सारा डेटा लाइव खींचकर लाता है"""
            ...
            async with session.get(url, headers=headers, timeout=10) as resp:
                if resp.status == 200:
                    ...
                    if content_b64:
                        ...
                        # ग्लोबल वेरिएबल्स में डेटा लोड करना
...
async def save_to_github():
    """लोकल वेरिएबल्स के डेटा को गिटहब की फाइल में ऑटो-कमिट कर देता है"""
    sha = await fetch_from_github() # लेटेस्ट SHA कोड पाना ज़रूरी है ताकि कमिट फेल न हो
    ...
    payload = {
        "message": "⚡ System Sync: Database auto-updated",
        "content": enc_content
    }
    if sha:
        payload["sha"] = sha
...
async def startup_event():
    await fetch_from_github() # सर्वर ऑन होते ही गिटहब से फ्रेश डेटा सिंक करें
...
<head>
    ...
    <style>
        ...
        .container {{
            ...
            box-sizing: border-box; position: relative; backdrop-filter: blur(10px);
        }}
        ...
        .brand-header {{
            ...
            animation: flowColors 6s linear infinite; text-shadow: 0 0 20px rgba(0, 229, 255, 0.4);
        }}
        ...
        .menu-trigger-btn {{
            ...
            padding: 6px 12px; border-radius: 4px; font-family: 'Courier New', Courier, monospace;
            font-size: 12px; font-weight: bold; cursor: pointer; transition: 0.2s;
        }}
        .menu-trigger-btn:hover {{ background: rgba(0, 229, 255, 0.2); box-shadow: 0 0 10px var(--secondary); }}
        ...
        .token-update-btn:hover {{ background: rgba(255, 170, 0, 0.2); box-shadow: 0 0 10px #ffaa00; }}
        ...
        .links-menu {{
            ...
            background: #090d16; border: 2px solid var(--accent); border-radius: 6px;
            width: 90%; max-width: 400px; max-height: 75vh; overflow-y: auto; padding: 20px;
            box-shadow: 0 0 30px var(--accent-glow); z-index: 1000; box-sizing: border-box;
        }}
        .menu-header {{ display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px; padding-bottom: 10px; border-bottom: 1px solid var(--accent); }}
        ...
        .menu-link-item {{
            ...
            text-decoration: none; font-size: 13px; font-weight: bold; display: flex; align-items: center; justify-content: space-between;
            border: 1px solid var(--terminal-border); transition: 0.2s;
        }}
        .menu-link-item:hover {{ background: rgba(255, 0, 119, 0.15); color: var(--accent); transform: translateX(4px); border-color: var(--accent); }}

        h1 {{ text-align: center; color: var(--secondary); font-size: 20px; margin: 5px 0; letter-spacing: 2px; text-shadow: 0 0 8px var(--secondary-glow); }}
        .subtitle {{ text-align: center; color: #64748b; font-size: 11px; margin-bottom: 25px; text-transform: uppercase; }}
        
        .stats-box {{
            background: rgba(13, 22, 43, 0.9); padding: 12px; border-radius: 4px; margin-bottom: 20px;
            display: flex; justify-content: space-between; font-size: 13px; border: 1px solid var(--terminal-border); border-left: 4px solid var(--primary);
        }}
        
        .input-group {{ margin-bottom: 18px; }}
        .input-group label {{ display: block; margin-bottom: 8px; font-size: 12px; color: var(--secondary); text-transform: uppercase; font-weight: bold; }}
        .input-group input, .input-group select {{
            width: 100%; padding: 12px; border-radius: 4px; border: 1px solid var(--terminal-border); background: #040812;
            color: #ffffff; box-sizing: border-box; font-size: 14px; font-family: 'Courier New', Courier, monospace;
        }}
        ...
        button {{
            ...
            cursor: pointer; transition: 0.2s; display: flex; align-items: center; justify-content: center; gap: 8px; text-transform: uppercase;
        }}
        ...
        .sub-modal-content {{
            background: #090f1c; border: 2px solid #ffaa00; border-radius: 8px; width: 100%; max-width: 440px; padding: 20px; box-sizing: border-box; position: relative;
        }}
        ...
        .section-title {{ color: var(--secondary); font-size: 13px; font-weight: bold; text-transform: uppercase; border-bottom: 1px dashed var(--terminal-border); padding-bottom: 4px; margin: 15px 0 8px 0; display: flex; align-items: center; gap: 6px; }}
        ...
        .info-sig {{ background: rgba(0, 255, 170, 0.02); padding: 10px; border-radius: 4px; font-style: italic; color: #94a3b8; font-size: 12px; border-left: 3px solid var(--secondary); }}
    ...
    <div class="sub-modal-content">
        ...
        <div class="qr-area" id="qrArea">
            ...
            <div class="input-group" style="margin-top:12px; text-align:left;">
                ...
                <input type="text" id="subUid" placeholder="Enter UID...">
            </div>
            <div class="input-group" style="text-align:left;">
                <label>TRANSACTION ID / UTR</label>
                <input type="text" id="subTxid" placeholder="Enter TXID...">
            </div>
...
<div class="container">
    ...
    <form id="tokenForm" style="margin-top: 15px;">
        <div class="input-group">
            <label><i class="fa-solid fa-user-secret"></i> GUEST ID</label>
            <input type="text" id="tokenUid" placeholder="Enter guest UID...">
        </div>
        <div class="input-group">
            <label><i class="fa-solid fa-key"></i> PASSWORD</label>
            <input type="password" id="tokenPassword" placeholder="Enter password...">
        </div>
...
<script>
    ...
    // ⚡ ATTRACTIVE NEON RGB MATRIX DIGITAL RAIN ENGINE ⚡
    ...
    function drawMatrix() {{
        ctx.fillStyle = 'rgba(2, 4, 10, 0.08)'; ctx.fillRect(0, 0, canvas.width, canvas.height);
    ...
    async function triggerManualTokenUpdate() {{
        ...
        }} catch(e) {{ alert("❌ टोकन अपडेट एपीआई टाइमआउट!"); }}
    }}
    ...
    async function submitSubRequest() {{
        ...
        if(!uid.trim() || !txid.trim()) {{ alert("UID और Transaction ID भरें!"); return; }}
        const formData = new FormData(); formData.append('uid', uid); formData.append('txid', txid); formData.append('amount', selectedAmount); formData.append('days', selectedDays);
        try {{
            ...
            if(data.status === 'success') {{ resultDiv.className = 'success-res'; resultDiv.innerHTML = "✅ " + data.message; }}
            else {{ resultDiv.className = 'error-res'; resultDiv.innerHTML = "❌ " + data.message; }}
        }} catch(e) {{ resultDiv.style.display = 'block'; resultDiv.className = 'error-res'; resultDiv.innerHTML = "❌ कनेक्शन फ़ेल हुआ।"; }}
    }}
    ...
    async function processAction(actionType) {{
        ...
        if (!uid.trim()) {{ alert("Target UID required!"); return; }}
        ...
        try {{
            ...
            if (data.status === 'success') {{
                if (actionType === 'like') {{
                    ...
                    resultDiv.removeAttribute('class'); let res = data.info;
                    resultDiv.innerHTML = '<div class="info-card"><div class="section-title">BASIC REGISTRY</div>' +
                        ...
                        '<div class="section-title">SIGNATURE KEY</div><div class="info-sig">' + res.signature + '</div></div>';
                }}
            }} else {{ resultDiv.className = 'error-res'; resultDiv.innerHTML = "❌ " + data.message; }}
        }} catch (error) {{ loader.style.display = 'none'; resultDiv.style.display = 'block'; resultDiv.className = 'error-res'; resultDiv.innerHTML = "❌ SYSTEM FAULT."; }}
    }}
    ...
    async function generateGuestToken() {{
        ...
        if (!uid.trim() || !password.trim()) {{ alert("UID and Password required!"); return; }}
        const formData = new FormData(); formData.append('uid', uid); formData.append('password', password);
        try {{
            const response = await fetch('/api/generate-token', {{ method: 'POST', body: formData }});
            const data = await response.json(); resultDiv.style.display = 'block';
            if (data.status === 'success') {{
                resultDiv.className = 'success-res';
                resultDiv.innerHTML = "<h3>✅ TOKEN CODES GENERATED!</h3><div class='raw-data-box'><pre>" + JSON.stringify(data.payload, null, 4) + "</pre></div>";
            }} else {{ resultDiv.className = 'error-res'; resultDiv.innerHTML = "❌ " + data.message; }}
        }} catch(e) {{ resultDiv.className = 'error-res'; resultDiv.innerHTML = "❌ ENDPOINT TIMEOUT."; }}
    }}
</script>
</body>
</html>
...
<body>
    <div class="header">
        ...
        <p>SECURE GITHUB DATABASE INTERFACE</p>
...
async def manual_token_update():
    ...
    return JSONResponse({"status": "error", "message": "टोकन सर्वर से रिस्पॉन्स नहीं मिला।"})
...
async def submit_subscription(uid: str = Form(...), txid: str = Form(...), amount: int = Form(...), days: int = Form(...)):
    ...
    for req in subscriptions["pending"]:
        if req["txid"] == txid:
            return JSONResponse({"status": "error", "message": "यह ID पहले से ही पेंडिंग है!"})
            
    subscriptions["pending"].append({"uid": uid, "txid": txid, "amount": amount, "days": days})
    await save_to_github()
    return JSONResponse({"status": "success", "message": "आपकी रिक्वेस्ट एडमिन पैनल में भेज दी गई है!"})
...
async def approve_sub(idx: int, username: str = Depends(admin_auth)):
    ...
    if 0 <= idx < len(subscriptions["pending"]):
        ...
        await save_to_github()
    return HTMLResponse("<script>alert('Approved & Saved to GitHub!'); window.location.href='/admin';</script>")
...
async def process(request: Request, region: str = Form(...), uid: str = Form(...), action: str = Form(...)):
    ...
    if not uid.isdigit():
        return JSONResponse({"status": "error", "message": "UID केवल अंकों में होनी चाहिए!"})
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(url, timeout=6) as resp:
                        if resp.status == 200:
                            ...
                            return JSONResponse({"status": "success", "info": clean_profile})
            except: pass
            await asyncio.sleep(0.1)
        return JSONResponse({"status": "error", "message": "सर्वर बिजी है, फिर प्रयास करें!"})

    elif action == "like":
        if not await can_user_like(client_ip):
                        ...
                        if resp.status == 200:
                            ...
                            if data.get('status') == 1:
                                ...
                                return JSONResponse({
                                    ...
                                    "uid": data.get('UID', uid), "given": data.get('LikesGivenByAPI', 0)
                                })
                            elif data.get('status') == 2:
                                return JSONResponse({"status": "error", "message": "इस UID की आज की लिमिट खत्म हो गई है।"})
            except: pass
            await asyncio.sleep(0.1)
        return JSONResponse({"status": "error", "message": "लाइक सर्वर ओवरलोड है!"})
...
async def generate_token(uid: str = Form(...), password: str = Form(...)):
    ...
    return JSONResponse({"status": "error", "message": "टोकन एपीआई डाउन है।"})
